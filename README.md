...
use Maven for that. Copy ready-to-use `pom.xml` for `node_modules/jasmine-allure-reporter` and run:
`mvn site -Dallure.results_pattern=allure-results`. It will put HTMLs into `target/site/allure-maven-plugin` folder.
To serve them via localhost:1324 use: `mvn jetty:run -Djetty.port=1234`
